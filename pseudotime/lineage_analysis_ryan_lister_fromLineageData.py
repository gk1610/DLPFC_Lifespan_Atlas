...
def lineage_analysis(lineageData, rand_state, hvgFile, n_pcs, n_neighbors, out_label, batch_key, rm_sex_chr_hvg = False):
	# step0. Load data
	...
	if (rm_sex_chr_hvg == "False"):
		high_adata = adata[:,adata.var.highly_variable.values]
	else:
		high_adata = adata[:,(adata.var.highly_variable.values & ~adata.var['gene_chrom-Aging'].isin(rm_sex_chr_hvg.split(";")))] # Need to be updated
	...
	print('PCA calculation is finished!')
	...
	# # step8. Leiden Clustering
	# sc.tl.leiden( high_adata, resolution=1.0, random_state=0)
	# sc.tl.rank_genes_groups( high_adata, 'leiden')
	# high_adata.write(out_label + '_high_adata.h5ad')

	# # step9. Misc Feature Plots
	# # subsampling the full adata will reproduce adata with random order
	# plt_adata = sc.pp.subsample( high_adata, rand_state=1.0, random_state=0, copy=True)
	# ax = sc.pl.umap( plt_adata, color=['stage_id','leiden','Batch'], palette = 'tab20', legend_fontsize=5, wspace=0.25, show=False)
	# plt.savefig(f"{out_label}_umap_stage_id_leiden_batch.png", format='png', bbox_inches='tight')
	...
	# # step10. Update full count matrix
	# # set umap and pca to adata
	# adata.obsm = high_adata.obsm
...
def main():
	...
	parser.add_option('-s', '--rm_sex_chr_hvg', dest='rm_sex_chr_hvg', help='Remove genes in sex chromosomes from hvgs?')

	(options,args) = parser.parse_args()
	lineage_analysis(options.lineageData, int(options.rand_state), options.hvgFile, options.numPCs, int(options.numNeighbors), options.out_label, options.batch_key, options.rm_sex_chr_hvg)
