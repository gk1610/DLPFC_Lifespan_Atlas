...
def whole_data_analysis_beforeUMAT(input_files, rand_state, batch_labels, out_label, down_sample, hvgInfo, n_pcs, n_neighbors, batch_key):
	# step0: Prepare data
...
def main():
	...
	print(options)
	whole_data_analysis_beforeUMAT(options.input_files, int(options.rand_state), options.batch_labels, options.out_label, options.down_sample, options.hvgInfo, options.n_pcs, int(options.n_neighbors), options.batch_key)
